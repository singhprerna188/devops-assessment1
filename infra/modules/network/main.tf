...
resource "aws_subnet" "public" {
  ...
  tags = merge(local.tags, { Name = "${local.name_prefix}-public-${var.azs[count.index]}" })
}
...
resource "aws_subnet" "private" {
  ...
  tags = merge(local.tags, { Name = "${local.name_prefix}-private-${var.azs[count.index]}" })
}

# NAT gateway (single NAT for cost efficiency; use one per AZ in a stricter prod setup)
...
resource "aws_security_group" "ecs" {
  ...
  egress {
    description = "All outbound (needed for ECR pulls, RDS access, etc.)"
    from_port   = 0
    to_port     = 0
    protocol    = "-1"
    cidr_blocks = ["0.0.0.0/0"]
...
# RDS SG: only accepts traffic from ECS tasks, on the DB port
